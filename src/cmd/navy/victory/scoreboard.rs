...
impl<'a> Request<'a> {
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        let (start, ordering) = match self.at {
            Locator::Top => {
                let v = match NavalVictoryCount::load_asc(&ctx.db_cfg, guild_id, 0, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
            ...
            Locator::Bottom => {
                let count = match NavalVictoryCount::count_rows(&ctx.db_cfg, guild_id).await {
                    Ok(c) => c,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                ...
                let mut a = match NavalVictoryCount::load_desc(&ctx.db_cfg, guild_id, 0, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Rank(r) => {
                let a = match NavalVictoryCount::load_asc(&ctx.db_cfg, guild_id, r, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Me => {
                let rank = match NavalVictoryCount::get_rank_of(&ctx.db_cfg, ctx.cmd.user.id, guild_id).await {
                    Ok(r) => r,
                    Err(e) => {
                ...
                let v = match NavalVictoryCount::load_asc(&ctx.db_cfg, guild_id, start, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Someone(u) => {
                let rank = match NavalVictoryCount::get_rank_of(&ctx.db_cfg, u, guild_id).await {
                    Ok(r) => r,
                    Err(e) => {
