...
impl<'a> Request<'a> {
    pub fn parse(_cmd: &'a CommandInteraction, options: &'_ [ResolvedOption<'a>]) -> Result<Self, RequestError> {
            ...
            match opt.name {
                "limit" => {
                    let ResolvedValue::Integer(lim) = opt.value else {
                        ...
                        return Err(RequestError::Internal("Bad value for `limit` in `legion kill scoreboard`.".into()));
                    };
                    if lim > 50 {
                        trc::error!("Bad value for `limit` in `legion kill scoreboard` {:?}", opt);
                ...
                "at" => {
                    let ResolvedValue::String(a) = opt.value else {
                        trc::error!("Bad value for `at` in `legion kill scoreboard` {:?}", opt);
                        return Err(RequestError::Internal("Bad value for `at` in `legion kill scoreboard`".into()));
                    };
                    at_discrim = a;
                },
                "someone" => {
                    let ResolvedValue::User(u, _) = opt.value else {
                        trc::error!("Bad value for `someone` in `legion kill scoreboard` {:?}", opt);
                        return Err(RequestError::Internal("Bad value for `someone` in `legion kill scoreboard`".into()));
                    };
                    someone = Some(u.id);
                },
                "rank" => {
                    let ResolvedValue::Integer(r) = opt.value else {
                        trc::error!("Bad value for `rank` in `legion kill scoreboard` {:?}", opt);
                        return Err(RequestError::Internal("Bad value for `rank` in `legion kill scoreboard`".into()));
                    };
                    rank = Some(r);
                },
                _ => {
                    trc::error!("Unknown option in `legion kill scoreboard` {:?}", opt);
                    return Err(RequestError::Internal("Unknown value for `rank` in `legion kill scoreboard`".into()));
                }
            }
        }
        ...
        let at = match at_discrim {
            ...
            _ => {
                return Err(RequestError::Internal("Unknown value for `at` in `legion kill scoreboard`.".into()));
            },
        };
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        let (start, ordering) = match self.at {
            Locator::Top => {
                let v = match LegionKillCount::load_asc(&ctx.db_cfg, guild_id, 0, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
            ...
            Locator::Bottom => {
                let count = match LegionKillCount::count_rows(&ctx.db_cfg, guild_id).await {
                    Ok(c) => c,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                ...
                let mut a = match LegionKillCount::load_desc(&ctx.db_cfg, guild_id, 0, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Rank(r) => {
                let a = match LegionKillCount::load_asc(&ctx.db_cfg, guild_id, r, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Me => {
                let rank = match LegionKillCount::get_rank_of(&ctx.db_cfg, ctx.cmd.user.id, guild_id).await {
                    Ok(r) => r,
                    Err(e) => {
                ...
                let v = match LegionKillCount::load_asc(&ctx.db_cfg, guild_id, start, self.limit).await {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Someone(u) => {
                let rank = match LegionKillCount::get_rank_of(&ctx.db_cfg, u, guild_id).await {
                    Ok(r) => r,
                    Err(e) => {
        ...
        for (offset, record) in ordering.into_iter().enumerate() {
            ...
            if record.kills != 1.into() {
                buffer.push_str(" kills\n");
            } else {
                buffer.push_str(" kill\n");
            }
        }
