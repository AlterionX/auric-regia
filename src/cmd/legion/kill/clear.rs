...
impl Request {
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        while let Some(active_records) = match LegionKillCount::load_asc(&ctx.db_cfg, current_offset, QUERY_LIMIT) {
            Ok(v) => if v.is_empty() {
                None
            } else {
        ...
        match LegionKillCount::delete(&ctx.db_cfg, self.deleter, records_to_delete.as_slice()) {
            Ok(count) => if count == 0 {
