...
use crate::{cmd::RequestError, db::IndustryProfitCount, discord::ExecutionContext};
...
impl<'a> Request<'a> {
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        let (start, ordering) = match self.at {
            Locator::Top => {
                let v = match IndustryProfitCount::load_asc(&ctx.db_cfg, 0, self.limit) {
                    Ok(v) => v,
                    Err(e) => {
            ...
            Locator::Bottom => {
                let count = match IndustryProfitCount::count_rows(&ctx.db_cfg) {
                    Ok(c) => c,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                ...
                let mut a = match IndustryProfitCount::load_desc(&ctx.db_cfg, 0, self.limit) {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Rank(r) => {
                let a = match IndustryProfitCount::load_asc(&ctx.db_cfg, r, self.limit) {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Me => {
                let rank = match IndustryProfitCount::get_rank_of(&ctx.db_cfg, ctx.cmd.user.id) {
                    Ok(r) => r,
                    Err(e) => {
                ...
                let v = match IndustryProfitCount::load_asc(&ctx.db_cfg, start, self.limit) {
                    Ok(v) => v,
                    Err(e) => {
                        trc::error!("Failed to get scoreboard items from top due to {e:?}.");
                        return Err(RequestError::Internal("failed to get scoreboard items".into()));
            ...
            Locator::Someone(u) => {
                let rank = match IndustryProfitCount::get_rank_of(&ctx.db_cfg, u) {
                    Ok(r) => r,
                    Err(e) => {
