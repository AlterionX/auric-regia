...
impl Request {
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        while let Some(active_records) = match IndustryProfitCount::load_asc(&ctx.db_cfg, current_offset, QUERY_LIMIT) {
            Ok(v) => if v.is_empty() {
                None
            } else {
            ...
            for record in active_records {
                let member = match self.guild.member(&ctx.ctx, record.id.to_u64().unwrap()).await {
                    ...
                    Err(SerenityError::Http(HttpError::UnsuccessfulRequest(e @ ErrorResponse { status_code: StatusCode::NOT_FOUND, error: DiscordJsonError { code: 10007, .. }, .. }))) => {
                        trc::info!("industry profit member check: {e:?}");
                        None
                    },
                    Err(e) => {
        ...
        match IndustryProfitCount::delete(&ctx.db_cfg, self.deleter, records_to_delete.as_slice()) {
            Ok(count) => if count == 0 {
