...
impl Request {
    ...
    pub async fn execute(self, ctx: &ExecutionContext<'_>) -> Result<(), RequestError> {
        ...
        while let Some(active_records) = match IndustryProfitCount::load_asc(&ctx.db_cfg, guild_id, current_offset, QUERY_LIMIT).await {
            Ok(v) => if v.is_empty() {
                None
            } else {
        ...
        match IndustryProfitCount::delete(&ctx.db_cfg, self.deleter, records_to_delete.as_slice()).await {
            Ok(count) => if count == 0 {
